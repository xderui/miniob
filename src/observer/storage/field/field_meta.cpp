...
const static Json::StaticString FIELD_ISNULL("isnull");

FieldMeta::FieldMeta() : attr_type_(AttrType::UNDEFINED), attr_offset_(-1), attr_len_(0), visible_(false)
{}

FieldMeta::FieldMeta(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool isnull)
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible, isnull);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}

RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, bool isnull)
{
...
void FieldMeta::to_json(Json::Value &json_value) const
{
  ...
  json_value[FIELD_ISNULL] = isnull_;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  const Json::Value &isnull_value = json_value[FIELD_ISNULL];
  ...
  if (!isnull_value.isBool()) {
    LOG_ERROR("Isnull ifield is not an bool value. json value=%s", isnull_value.toStyledString().c_str());
  }
  ...
  bool isnull = isnull_value.asBool();
  return field.init(name, type, offset, len, visible, isnull);
}
